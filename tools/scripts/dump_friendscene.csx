...
foreach(var ob in new[]{"obj_flowey_friendscene","obj_friendlyflowey","obj_flowey"}){
  var codes=Data.Code.Where(x=>x.Name?.Content!=null&&x.Name.Content.StartsWith("gml_Object_"+ob+"_")).OrderBy(x=>x.Name.Content).ToList();
...
File.WriteAllText(@"C:\Users\jojoc\undertale-3ds\extracted\friendscene.txt",sb.ToString());
Console.WriteLine("done len="+sb.Length);
